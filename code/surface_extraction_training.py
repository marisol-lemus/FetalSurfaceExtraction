...
def load_data(args, data_split='train'):
    ...
    for i in tqdm(range(len(subject_list))):
        ...
        # ------- load surface -------
        if surf_type == 'white':
            surf_in = nib.load('FetalSurfaceExtraction/template/init_surf_'+surf_hemi+'.surf.gii')
            surf_gt = nib.load(data_path+subid+'/'+subid+'_'+'hemi_'+surf_hemi+'_wm.surf.gii')
            affine_in = vol_mni.affine
            affine_gt = vol.affine
        elif surf_type == 'pial':
            surf_in = nib.load(data_path+subid+'/'+subid+'_'+'hemi_'+surf_hemi+'_wm.surf.gii')
            surf_gt = nib.load(data_path+subid+'/'+subid+'_'+'hemi_'+surf_hemi+'_pial.surf.gii')
